...
int main()
{
    ...
    plant.worldCell = world.getCellIndex(z, x, mapScale); //gives plant knowledge of its cell. Z BEFORE X
    cout << "PLACED FIRST PLANT AT CELL " << plant.worldCell << endl;
    ...
    int mapSeed = 1904;   //10363 mountains
    ...
    int plantEatCounter = 0;
    int plantReproduceCounter = 0;
    int worldUpdateCounter = 0;

    /*GAME LOOP UPDATE INTERVALS*/
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        */
        ...
        if (addPlant) {
            ...
            //float curX = plantLocations[plindex].x + 3.4;
           // float curZ = plantLocations[plindex].z + 2.5; //can place along Z-axis! (horizontal), only if ON axis
            float curX = plantVector[plantVector.size() - 2].coordinates.x + 30.4;
            float curZ = plantVector[plantVector.size() - 2].coordinates.z + 20.6;
            float curY = map.getHeight(curZ, curX, mapScale);
            //plantLocations.push_back(glm::vec3(curX, curY, curZ ));
            plantVector[plantVector.size()-1].coordinates = glm::vec3(curX, curY, curZ);
        ...
        */
        ...
        if (plantReproduceCounter > 3000 && pause == false) {
            //cout << "SEX & DEATH" << endl;
            for (int i = 0; i < plantVector.size(); i++) {
                if (plantVector[i].checkReproduce() == 1 && plantVector.size() < MAX_PLANTS) {
                    //addPlant = true; //replace with asexual reproduction
                    Plant child(true); //create an empty plant
                    plantVector[i].asexualReproduction(&child); //populate child with parents genetics
                    ...
                    if (child.coordinates.x < 0 || child.coordinates.z < 0 || child.coordinates.x > mapScale*0.99
                        ...
                        //need to reflect seed back into map (prevent loss of fitness)
                        //FOR NOW
                        cout << "WARNING: CHILD OUTSIDE OF BOUNDS\n";
                        //child.coordinates.x = 600.0;
                        //child.coordinates.z = 600.0;
                        child.~Plant(); //destroy this child
                    }
                    else {
                        //calculate the map height of the child
                        cout << "[main]getHeight z=" << child.coordinates.z << ", x=" << child.coordinates.x << endl;
                        ...
                        //cout << "added child to the world at " << child.coordinates.x << "," << child.coordinates.y <<
                            //", " << child.coordinates.z;

...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (plantEatCounter > 1000) {
            cout << "EAT" << endl;
            for (int i = 0; i < plantVector.size(); i++) {
                int lightLevel = 2000; //fetch from WORLD CLASS later on
                plantVector[i].eat(lightLevel); //photosynthesize
        ...
        plantEatCounter += 1;
        if (plantReproduceCounter > 10000) {
            cout << "SEX & DEATH" << endl;
            for (int i = 0; i < plantVector.size(); i++) {
                if (plantVector[i].checkReproduce() == 1) {
                    addPlant = true;
                }

...
//SIMULATION TIME
bool pause = true; //stop lifecycle

//Plant Life 
const int MAX_PLANTS = 600;
...
int main()
{
    ...
    float smooth = 1.0; //higher -> bumpier.  closer to 0 -> flatter
    int mapSeed = 11;   //10363 mountains //1904 nice variation //1997 mtn valley
    ...
    glm::mat4 projection;
    projection = glm::perspective(glm::radians(45.0f)/*FOV*/, 1200.0f / 800.0f/*aspect ratio*/, 0.1f, 2000.0f);
    ...
    float avgNumStems = 0;
    float avgGrowthSpeed = 0;
    float avgWidth = 0;
    float avgSeedEnergy = 0;
    float avgWaterCost = 0; //avg water cost in stage 6

    /*RUNTIME CONTROLS*/
    float simSpeed = 0.05; //how fast our simulation runs.  closer to 0 is faster
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glClearColor(0.7f, 0.7f, 0.9f, 1.0f); // old teal: 0.2f, 0.3f, 0.3f, 1.0f
       ...
       /*========REPRODUCTION========*/
        if (!pause) {
            ...
            if (plantVector[i].checkReproduce() == 1 && plantVector.size() < MAX_PLANTS) {
                seed += 1;
                plantVector.emplace_back(Plant(true, seed));
        ...
        if (statCounter > 2502) {
            ...
            float sumWidth = 0, sumNumStems = 0;
            float sumGrowthSpeed = 0, sumSeedEnergy = 0, sumWaterCost = 0;
            for (int i = 0; i < plantPop; i++) {
                sumGrowthSpeed += plantVector[i].growthSpeed;
                sumWidth += plantVector[i].turtle.width;
                sumNumStems += plantVector[i].numStems;
                sumSeedEnergy += plantVector[i].seedEnergy;
                sumWaterCost += plantVector[i].waterCost[6];
            }
            avgGrowthSpeed = sumGrowthSpeed / plantPop;
            avgNumStems = sumNumStems / plantPop;
            avgWidth = sumWidth / plantPop;
            avgSeedEnergy = sumSeedEnergy / plantPop;
            avgWaterCost = sumWaterCost / plantPop;
            cout << "avgGS: " << avgGrowthSpeed << endl;
            cout << "avgWidth " << avgWidth << endl;
            cout << "avgNumStems " << avgNumStems << endl;
            cout << "avgSeedEnergy " << avgSeedEnergy << endl;
            cout << "avgWaterCost " << avgWaterCost << endl;
